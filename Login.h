...
public:
	...
	bool idEqual(string str) {
		...
		for (unsigned int i = 0; i < vctr.size(); i++) {
			if (str == vctr[i][1]) { //1번 자리에 ID를 입력하기에 이와 같이 작성
				tmpID = str;
				userNum = i;
				return true;
			}
	...
	void doLogin(string str1, string str2) {
		if (idEqual(str1) == true && pwEqual(str2) == true) {
			currentUserNum = userNum; //////////user ID로 관리한다..... num으로 하면, 수정시에 매우 피곤해진다.
			currentUserID = vectorRead(currentUserNum, "ID");
			cout << "로그인합니다." << endl;
			cout << "유저 번호 : " << currentUserNum << " ID : " << vctr[userNum][1] << " PW : " << vctr[userNum][2] << endl;
			cout << "현재 로그인된 ID : " << currentUserID << endl;

		}
		else {
			cout << "로그인 조건 불만족" << endl;
		}

	}
	void logOut() {
		cout << "현재 유저번호 : " << currentUserNum << endl;
		cout << "로그아웃" << endl;
		currentUserNum = 9999;
		currentUserID = "null";
	...
	void idRegister(string id, string pw) {  ///////////////회원가입! 유저번호, ID, PW를 인자로 넣는다
		if (idEqual(id) == true) {
			...
			vector<string> vct;
			string tmpstr = "";
			tmpstr = to_string(vctr.size() + 1);  ////int string 형변환!
	...
	void readDemo() { ///////////// 파일출력 :: User.csv에서 읽어와서 벡터에 넣어버린다.
